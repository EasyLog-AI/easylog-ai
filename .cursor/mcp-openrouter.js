...
/**
 * OpenRouter MCP Server for Cursor
 * Provides OpenRouter AI model access via Model Context Protocol
 */
...
class OpenRouterServer {
      ...
      tools: [
        {
          ...
          inputSchema: {
            ...
            properties: {
              model: {
                type: "string",
                description: "OpenRouter model name (e.g., 'anthropic/claude-sonnet-4', 'openai/gpt-4.1')",
  ...
  async handleChatCompletion(args) {
    const { model = "openai/gpt-4.1", messages, max_tokens = 1000, temperature = 0.7, top_p = 1 } = args;
